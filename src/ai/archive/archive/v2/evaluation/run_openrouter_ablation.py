...
ABLATION_OUT = "src/ai/quiz-engine/v2/evaluation/openrouter_visual_ablation_results.json"
REPORT_OUT = "src/ai/quiz-engine/v2/reports/openrouter_visual_ablation_report.md"
...
def make_request_with_retry(payload, headers, max_retries=2):
    delays = [3, 8]
    for attempt in range(max_retries + 1):
        start = time.time()
        try:
            res = requests.post(OPENROUTER_CHAT_URL, headers=headers, json=payload, timeout=(10, 30))
            res.raise_for_status()
            lat = int((time.time() - start) * 1000)
            return res.json()["choices"][0]["message"]["content"], lat, None
            
        except requests.exceptions.HTTPError as e:
            status_code = e.response.status_code
            if status_code in [401, 402, 403]:
                return None, 0, f"HTTP {status_code}: {e.response.text}"
            
            if attempt < max_retries:
                time.sleep(delays[attempt])
                continue
            return None, 0, f"HTTP {status_code} after {max_retries} retries"
            
        except (requests.exceptions.Timeout, requests.exceptions.ConnectionError) as e:
            if attempt < max_retries:
                time.sleep(10)
                continue
            return None, 0, f"Timeout/Connection error after {max_retries} retries: {str(e)}"
            
        except Exception as e:
            if attempt < max_retries:
                time.sleep(delays[attempt])
                continue
            return None, 0, f"Unknown error after {max_retries} retries: {str(e)}"
...
def main():
    ...
    print("\n--- RUNNING FULL ABLATION (RESUMABLE) ---\n")
    ...
    for idx, item in enumerate(image_mcqs):
        ...
        prompt_text = f"Answer this multiple-choice question. Return ONLY A, B, C, or D.\n\nQuestion:\n{item['question']}\n\nA. {item['options'][0]}\nB. {item['options'][1]}\nC. {item['options'][2]}\nD. {item['options'][3]}"
        ...
        if existing["image_prediction"] is None or existing["image_prediction"] == "ERROR":
            ...
            p_img = {
                "model": MODEL_ID,
                "messages": [{"role": "user", "content": [{"type": "image_url", "image_url": {"url": url}}, {"type": "text", "text": prompt_text}]}],
                "max_tokens": 20,
                "temperature": 0.01
            }
            raw, lat, err = make_request_with_retry(p_img, headers)
            if err:
                existing["image_prediction"] = None
                existing["image_correct"] = None
                existing["image_raw_output"] = f"ERROR: {err}"
                existing["image_invalid"] = False # Do not count API errors as parsing invalids
        ...
        if existing["text_only_prediction"] is None or existing["text_only_prediction"] == "ERROR":
            ...
            raw, lat, err = make_request_with_retry(p_txt, headers)
            if err:
    ...
    # Accuracy over existing predictions individually
    ...
    # Gain is only over paired completed cases
    ...
    if (failed + partial) > (total * 0.5):
        ablation_status = "INCOMPLETE_DUE_TO_PROVIDER_AVAILABILITY"

    with open(REPORT_OUT, "w") as f:
        f.write("# OpenRouter Visual Ablation Report\n\n")
        ...
        f.write(f"- **NONE_VISUAL_CASES** = {none} (Both conditions correctly answered the question; the text alone was sufficient.)\n")
